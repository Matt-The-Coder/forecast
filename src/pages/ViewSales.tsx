...
export const ViewSales = () => {
    const { importSalesData, setImportSalesData } = useOutletContext<OutletContextType>()
    ...
    const data = importSalesData?.slice(firstIndexPage, lastIndexPage)
    const totalPages = Math.ceil(importSalesData.length / itemsPerPage)
    ...
    const addData = (e:any) => {
        e.preventDefault()
        if(ProductName && Price && DateOrdered && QuantityOrdered){
            if(importSalesData.length !== 0){
                setImportSalesData((prev)=>{return [...prev, {  
                    "Date Ordered": formatDate(DateOrdered),
                    "Product Name": ProductName,
                    "Product Price": Price,
                    "Quantity Ordered": QuantityOrdered,
                    Total: Price * QuantityOrdered}]})
                let importedItemWithNewlyAdded = importSalesData
                localStorage.setItem("importSalesData", JSON.stringify(importedItemWithNewlyAdded));
    ...
    const removeData = (index:number) => {
       const newData = importSalesData.filter((_, i)=>i!==index)
       localStorage.setItem("importSalesData", JSON.stringify(newData));
       const newStorageUpdate = localStorage.getItem("importSalesData");
       if(newStorageUpdate){
        setImportSalesData(JSON.parse(newStorageUpdate))
       }
                                        ...
                                        <div className="px-6 py-4 grid gap-3 md:flex md:justify-between md:items-center border-b border-gray-200 dark:border-neutral-700">
                                            <div>
                                                ...
                                                <p className="text-sm text-gray-600 dark:text-neutral-400">
                                                    Create sales, edit, download and filter.
                                                </p>
                                            </div>
                                            <div>
