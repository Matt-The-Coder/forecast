...
export const ViewItems = () => {
    ...
    const data = importItemsData?.slice(firstIndexPage, lastIndexPage)
    const totalPages = Math.ceil(importItemsData.length / itemsPerPage)
    ...
    const deleteData = () => {
        let dataFromLocal = localStorage.getItem("importItemsData");
        if(dataFromLocal?.length!==0){
            localStorage.removeItem("importItemsData")
            setImportItemsData([])
        }
    }
    useEffect(() => {
        let dataFromLocal = localStorage.getItem("importItemsData");
        if (importItemsData.length !== 0) {
            localStorage.setItem("importItemsData", JSON.stringify(importItemsData))
            let data = localStorage.getItem("importItemsData");
            if (data) {
                setImportItemsData(JSON.parse(data))
            }
        }
        else{
            if (dataFromLocal) {
    ...
    const removeData = (index:number) => {
       const newData = importItemsData.filter((_, i)=>i!==index)
       localStorage.setItem("importItemsData", JSON.stringify(newData));
       const newStorageUpdate = localStorage.getItem("importItemsData");
       if(newStorageUpdate){
        setImportItemsData(JSON.parse(newStorageUpdate))
       }
    ...
    const addData = (e:any) => {
        ...
        if(ProductName && Price){
            if(importItemsData.length !== 0){
                setImportItemsData((prev)=>{return [...prev, {Product:ProductName, Price: Price}]})
                let importedItemWithNewlyAdded = importItemsData
                localStorage.setItem("importItemsData", JSON.stringify(importedItemWithNewlyAdded));
        ...
        if (file) {
            ...
            Papa.parse(file, {
                ...
                complete: (results: any) => {
                    setImportItemsData(results.data); // Store parsed data in state
                                        ...
                                        <div className="px-6 py-4 grid gap-3 md:flex md:justify-between md:items-center border-b border-gray-200 dark:border-neutral-700">
                                            <div>
                                                ...
                                                <p className="text-sm text-gray-600 dark:text-neutral-400">
                                                    Create items, edit, download and filter.
                                                </p>
                                            </div>
                                            <div>
